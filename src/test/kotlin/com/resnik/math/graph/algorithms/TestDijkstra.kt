...
class TestDijkstra {
    ...
    fun testDijkstra(){
        ...
        val dijkstra = Dijkstra(GAParams(vertices[0], vertices[5]))
    ...
    fun testAStar(){
        ...
        edges.addAll(vertices[6].connectMultiple(vertices[5], vertices[2], vertices[4]))
        val graph = Graph(vertices.toSet(), edges)
        val astar = AStar(GAParams(vertices[0], vertices[5]))
        val path = astar.evaluate()
        println(path)
        val collection: GraphCollection = GraphCollection("AStar")
        collection.addGraph(graph)
        collection.addPath(path)
        collection.render()
    }
